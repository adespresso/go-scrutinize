...
func metalinter() {
	...
	if _, err = os.Stat("go-scrutinize.config"); os.IsNotExist(err) {
		cmd = exec.Command(goMetaLinterCmd, "./...", "--checkstyle", "--deadline=2m")
	} else {
		cmd = exec.Command(goMetaLinterCmd, "./...", "--checkstyle", "--deadline=2m", "--config=go-scrutinize.config")
	}
